...
export default async (req) => {
  ...
  const idArray = Object.keys(items[columnName]).map((key) =>
    key.replace(/^.*?:/, ""),
  );
  ...
  const postUrl =
    endpoint + relativeUrl + "?token=" + token + "&kg=wikidata&lang=en";
  ...
  if (res.status !== 200) {
    console.log(
      `*** request labels ### ERROR status code returned by LamAPI is: ${res.status} `,
    );
    return {};
  } else {
    console.log(
      `*** request labels ### status code returned by LamAPI is: ${res.status} - ${JSON.stringify(res.data)}`,
