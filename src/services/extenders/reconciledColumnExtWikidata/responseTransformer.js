...
export default async (req, res) => {
  ...
  console.log(`*** Label Extender *** inputColumn: ${JSON.stringify(inputColumnName)}`);
  ...
  for (const key in wikidata) { // for each entity
    // console.log(`*** Label Extender *** key: ${JSON.stringify(key)}`);
    // console.log(`*** Label Extender *** items: ${JSON.stringify(items)}`);
    const rowIdKey = items.buyer[`wd:${key}`]; // all rows with entity identified by key
    // console.log(`*** Label Extender *** entityIdKey: ${JSON.stringify(rowIdKey)}`);
    for (const row of rowIdKey) { // for each row with entity identified by key
      for (const property of props.labels) {  // for each requested property
        ...
        switch (property) {
          case "id":
            ...
            break;
          case "url":
            newCellContent.label = wikidata[key].url;
            break;
          case "name":
            newCellContent.label = wikidata[key].labels.en;
            break;
          case "description":
            newCellContent.label = wikidata[key].description;
        }
        ...
        response.columns[newColumnName].cells = { ...response.columns[newColumnName].cells, ...newCell };
      }
    }
  }
