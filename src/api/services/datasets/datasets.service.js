    ...
    addDataset: async (filePath, datasetName, userId) => {
        ...
        await writeQueue.push(async () => {
            try {
                ...
                // add dataset entry
                await writeFile(getDatasetDbPath(), JSON.stringify({
                    meta: metaDatasets,
                    datasets: {...datasets, ...newDatasets}
                }, null, 2));
                // add table entries
                await writeFile(getTablesDbPath(), JSON.stringify({
                    meta: metaTables,
            ...
            acc[rowId] = {
                ...
                cells: Object.keys(rawRows[rowId].cells).reduce((accCell, colId) => {
                    const {
                        ...
                        match,
                        metadata
                    } = FileSystemService.transformMetadata(rawRows[rowId].cells[colId].metadata);
